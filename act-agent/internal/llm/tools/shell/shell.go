...
func (s *PersistentShell) execCommand(command string, timeout time.Duration, ctx context.Context) commandResult {
	...
	tempDir := os.TempDir()
	stdoutFile := filepath.Join(tempDir, fmt.Sprintf("act-stdout-%d", time.Now().UnixNano()))
	stderrFile := filepath.Join(tempDir, fmt.Sprintf("act-stderr-%d", time.Now().UnixNano()))
	statusFile := filepath.Join(tempDir, fmt.Sprintf("act-status-%d", time.Now().UnixNano()))
	cwdFile := filepath.Join(tempDir, fmt.Sprintf("act-cwd-%d", time.Now().UnixNano()))
