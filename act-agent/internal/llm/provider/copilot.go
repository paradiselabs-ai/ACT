...
import (
	...
	toolsPkg "github.com/paradiselabs-ai/ACT/act-agent/internal/llm/tools"
...
func (c *copilotClient) exchangeGitHubToken(githubToken string) (string, error) {
	...
	req.Header.Set("User-Agent", "ACT/1.0")
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in act.json, or ensure GitHub CLI/Copilot is properly authenticated.")
	...
	openaiClientOptions = append(openaiClientOptions,
		option.WithHeader("Editor-Version", "ACT/1.0"),
		option.WithHeader("Editor-Plugin-Version", "ACT/1.0"),
