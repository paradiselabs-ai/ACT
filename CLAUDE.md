...
**Per-role backend selection (Tier 2).** Each swarm role can use one of several backends — **`runner/swarm_roles.go::IsValidBackend` is the source of truth; re-grep it, don't trust this list**:
...
- `antigravity` — the agy CLI (`agy --dangerously-skip-permissions --print`); **rejected for researcher** (no read-only/plan mode)
- `devin` — Devin CLI (`devin --permission-mode dangerous --respect-workspace-trust false -p`); **rejected for researcher** (no read-only/plan mode, no tool-restriction flag)

All of them are stateless one-shots per task — the Runner rebuilds full context (identity line, project brief, inbox, validation gaps, parallel-agent awareness) into every task prompt; no conversational memory across tasks by design (the ACT server is the memory).
...
Dual-purpose semantic memory: team coordination patterns + individual agent skill profiles. ChronLog + vector store. Evidence-based routing was **structurally broken** until 2026-08-19 (a 2026-08-13 live audit proved the routing brief fabricated a `developer` row and could never populate similar-projects: outcome events carried no project tag, and attribution joined the wrong event). **Fixed 2026-08-19** (uncommitted at time of writing): outcome events project-tagged at emit + legacy taskId→project fallback in the indexer; attribution joins `task_completed` first; search has a measured 0.28 relevance floor; the Runner now saves a per-agent Recent-Work brief after each completed task. Live-verified — routing brief returns real roles/pass-rates and non-empty similar projects on both a clean harness project and the real event history. History + repro: [docs/audits/memory-system-audit-2026-08-13.md](docs/audits/memory-system-audit-2026-08-13.md) (see its 2026-08-19 postscript). Known open: point-id same-millisecond collision (`pvm-point-id-collision-same-ms-2026-08-19`), synergy definition (`pvm-synergy-shared-project-definition-2026-08-19`).
...
3. **Coordination KG** — **NOT deferred; it exists.** `server/src/services/GraphStore.ts` + `GraphIndexer.ts` derive typed edges from ChronLog into `server/data/graph-edges.jsonl` — bi-temporal (`validAt`, `episodeKeys`, active/inactive edges), verified live on 2026-08-13 (164 edges / 104 nodes on disk; a replay produced 192 edges / 121 nodes). Surfaced via `/api/graph/*` and `act-agent graph task|unverified|conflicts`. No graph DB — a JSONL edge log. Neo4j/Kuzu still deferred until FLUX needs causal traversal.
...
7. **LocalEmbeddingVectorStore is already active, and as of 2026-06 its analytics are REAL too.** `getAgentProfile`/`compareAgents`/`getAgentSynergy` compute from actual task-outcome events on the active store; the old `0.85 + Math.random()` placeholders survive ONLY in the inactive Mock/Qdrant stores. When a user asks "is PVM real": embeddings real, analytics real-on-the-active-store. **But "computed, not faked" ≠ "correct":** the 2026-08-13 live audit proved the analytics read a broken join and an untagged project bucket, so they return confidently-wrong numbers (fake `developer` role, missing failures, permanently empty similar-projects list). Do NOT re-implement the analytics layer — fix its inputs (see [docs/audits/memory-system-audit-2026-08-13.md](docs/audits/memory-system-audit-2026-08-13.md) and the `pvm-*-2026-08-13` kanban tickets).
